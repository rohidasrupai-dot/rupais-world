...
for(const file of ['studio/store.js','studio/adaptive-teaching-intelligence-service.js','studio/ai-provider-service.js','studio/curio-conversation-service.js','studio/curio-conversation-orchestrator.js'])vm.runInContext(fs.readFileSync(file,'utf8'),context,{filename:file});
...
(async()=>{
  ...
  const noProvider=C.createConversation({userId:'neutral_user'});out=await O.orchestrate(noProvider.id,'neutral_user','Tell me an unverified fact about an unknown planet',{mode:'adaptive',explanationLevel:'adaptive'});assert.equal(out.state,'provider_unavailable');assert.equal(out.message.content,'AI conversation provider is not configured. Curio can still help using approved lessons and learning tools.');assert.equal(out.adaptiveDecision.localOnly,true);assert.equal(out.adaptiveDecision.providerUsed,false);
