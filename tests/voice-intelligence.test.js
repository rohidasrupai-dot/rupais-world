...
(async()=>{
  ...
  assert.equal(V.providerStatus().configured,false);assert.match(V.providerStatus().message,/not configured/);
  assert.equal(V.pauseSession(voice.id,actor.userId,actor).status,'paused');assert.equal(V.resumeSession(voice.id,actor.userId,actor).status,'active');
  ...
  V.savePreference(actor.userId,{inputLanguage:'hindi',outputLanguage:'hindi',personality:'teacher_curio',volume:.7,autoSend:false,readSummaryOnly:true,stopSpeechWhenTyping:true},actor);assert.equal(V.preference(actor.userId).autoSend,false);assert.equal(V.preference(actor.userId).personality,'teacher_curio');assert.equal(V.preference(actor.userId).volume,.7);assert.equal(V.availableVoices('hindi')[0].id,'voice-hi');
  ...
  const reading=V.lessonReading('lesson_voice');assert.equal(reading.available,true);assert.equal(reading.paragraphs[0].id,'section1');
  const readParagraph=await V.readLesson(voice.id,actor.userId,'lesson_voice',actor,{paragraphId:'section1'});assert.equal(readParagraph.state,'speaking');assert.equal(readParagraph.paragraphs.length,1);assert.equal(synth.current.volume,.7);
  assert.equal(V.repeatPrevious(voice.id,actor.userId,actor).state,'speaking');V.stopSpeaking(voice.id,actor.userId,actor);
  assert.equal(V.interpretCommand('Please open revision').command,'open_revision');assert.equal(V.interpretCommand('dheere bolo').command,'slower');assert.equal(V.interpretCommand('unrelated words').recognized,false);
  const previousRate=V.preference(actor.userId).speakingRate,command=V.executeCommand(voice.id,actor.userId,'slower',actor);assert.equal(command.executed,true);assert.ok(V.preference(actor.userId).speakingRate<previousRate);assert.equal(command.providerUsed,false);
  assert.equal(V.executeCommand(voice.id,actor.userId,'open notes',actor).result.navigation,'open_notes');
  ...
  assert.equal(V.stopSession(voice.id,actor.userId,actor).status,'stopped');
  ...
  console.log('Voice intelligence tests passed: session lifecycle, browser/provider/text-only modes, consent, permission, listening, partial/final/edit/confirm, English/Hindi/Hinglish settings, personalities, rate/pitch/volume, lesson and paragraph reading, repeat, local commands, playback controls, turn-taking, child safety, retention and honest provider status.');
