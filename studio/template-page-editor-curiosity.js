(async function () {
  ...
  async function renderCanvas() {
    $('#codedCanvas').innerHTML = isCuriosity ? curiosityMarkup() : earthMarkup();
    ...
    if (isCuriosity) selectZone(activeRichKey, false);
  }
  ...
  $('#codedCanvas').addEventListener('input', event => { const zone = event.target.closest('[data-rich-slot]'); if (!zone) return; page.content[zone.dataset.richSlot] = zone.innerHTML; queueSave(); });
  ...
  await renderCanvas(); await renderControls(); if (isCuriosity) selectZone('mainTitle', false);
