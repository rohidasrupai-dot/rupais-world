(function(){
  ...
  async function ask(conversationId,userId,question,options={}){
    ...
    const context=buildContext(c.id,userId,options.contextQuestion||text,options),state=ensure(),conversation=state.curioConversations.find(x=>x.id===c.id),planData=responsePlan(conversation,intent,context),plan={id:uid('response_plan'),conversationId:c.id,messageId:student.id,...planData,createdAt:now()};state.curioResponsePlans.push(plan);
    ...
    if(!route.shouldCallAI||navigator.onLine===false){const offline=navigator.onLine===false,copy=offline?'You are offline. Your question is saved, but new AI generation is unavailable. Approved lessons remain available.':'AI conversation provider is not configured. Curio can still help using approved lessons and learning tools.';const m=addMessage(state,conversation,{role:'curio',messageType:'no_provider_notice',content:copy,groundingState:'provider_unavailable',verificationState:'provider_unavailable',safetyResult:safety});conversation.status='provider_unavailable';state.curioPendingQuestions.push({id:uid('pending_question'),conversationId:c.id,messageId:student.id,userId,question:text,status:offline?'pending_connection':'needs_creator',createdAt:now()});queueReview(state,m,'unanswered_question',text);S().write(state);return{state:offline?'offline':'provider_unavailable',message:clone(m),intent,plan,context:context.snapshot}}
